...
export const RANKED_PLACEMENT_MATCHES = 3;
export const RANKED_FIXED_ROUND_COUNT = 10;

export const RANKED_LP_BY_PLACEMENT: Record<number, number> = {
  1: 30,
  2: 15,
  3: 0,
  4: -15,
...
export const RANKED_PLACEMENT_POINTS: Record<number, number> = {
  1: 3,
  2: 2,
  3: 1,
  4: 0,
...
export function getRankedDivision(lp: number): RankedDivision {
  const safeLp = Math.max(0, Math.floor(lp));
  return [...RANKED_DIVISIONS].reverse().find((rank) => safeLp >= rank.minLp) ?? RANKED_DIVISIONS[0];
}

export function getPlacementStartingLp(points: number): number {
  if (points >= 9) return 700;
  if (points === 8) return 600;
  if (points === 7) return 500;
  if (points === 6) return 400;
  if (points === 5) return 300;
