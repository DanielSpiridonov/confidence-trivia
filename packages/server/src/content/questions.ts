import { DIFFICULTY_REWARDS, QuestionRecord, Locale } from "@confidence-trivia/shared";
import { GENERATED_QUESTIONS } from "./generatedQuestions";

const ENABLED_TEST_TYPES = new Set<QuestionRecord["type"]>([
  "multiple_choice",
  "true_false",
  "estimate",
  "closest_answer",
 ...
 */
const SEED_QUESTIONS: QuestionRecord[] = [
  {
    id: "q1",
...
export const QUESTIONS: QuestionRecord[] = [...SEED_QUESTIONS, ...GENERATED_QUESTIONS].map((question) => ({
  ...question,
  basePoints: DIFFICULTY_REWARDS[question.difficulty],
}));

export function validateQuestionBank(questions: QuestionRecord[] = QUESTIONS): void {
  ...
  for (const question of questions) {
    ...
    if (question.basePoints !== DIFFICULTY_REWARDS[question.difficulty]) {
    ...
    for (const locale of ["en", "bg"] as const) {
      ...
      if (!translation?.text.trim()) throw new Error(`Missing ${locale} text for ${question.id}`);
      ...
      if (localizedQuestions.has(fingerprint)) throw new Error(`Duplicate ${locale} question: ${question.id}`);
 ...
 */
export function getQuestionSet(count: number, excludeIds: string[] = []): QuestionRecord[] {
  const enabledQuestions = QUESTIONS.filter((q) => ENABLED_TEST_TYPES.has(q.type));
  const questionSet: QuestionRecord[] = [];
  const excluded = new Set(excludeIds);
  const usedIds = new Set<string>();
  const typeCounts = new Map<QuestionRecord["type"], number>();
  const difficultyPattern: QuestionRecord["difficulty"][] = ["easy", "medium", "easy", "medium", "hard"];

  while (questionSet.length < count && enabledQuestions.length > 0) {
    if (usedIds.size >= enabledQuestions.length) usedIds.clear();
...
export function localize(record: QuestionRecord, locale: Locale) {
  ...
  return {
    ...
    text: t.text,
    options: t.options,
    basePoints: DIFFICULTY_REWARDS[record.difficulty],
  };
}

export function localizeAnswer(record: QuestionRecord, locale: Locale, answer: unknown): string {
  const t = record.translations[locale] ?? record.translations.en;
...
export function localizeAnswerItems(record: QuestionRecord, locale: Locale, answer: unknown): string[] {
  const t = record.translations[locale] ?? record.translations.en;
...
export function getLocalizedCorrectAnswer(record: QuestionRecord, locale: Locale): unknown {
