import { DIFFICULTY_REWARDS, QuestionRecord } from "@confidence-trivia/shared";
...
const multipleChoiceQuestions: QuestionRecord[] = Array.from({ length: 20 }, (_, index) => {
  ...
  return {
    ...
    category: "math",
    difficulty,
    basePoints: DIFFICULTY_REWARDS[difficulty],
...
] satisfies Array<[number, string, string, string]>;

const closestAnswerQuestions: QuestionRecord[] = closestFacts.map(([answer, enText, bgText, category], index) => ({
  id: `generated_closest_${index + 1}`,
  type: "closest_answer",
  category,
  difficulty: difficultyFor(index),
  basePoints: DIFFICULTY_REWARDS[difficultyFor(index)],
  correctAnswer: answer,
  translations: {
