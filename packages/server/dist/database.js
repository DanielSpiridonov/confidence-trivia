...
exports.anonymizePlayerAccount = anonymizePlayerAccount;
...
exports.canAccessPlayerData = canAccessPlayerData;
...
async function submitPlayerReport(reporterId, rawReportedName, rawDescription) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not submit player report", error);
...
async function getNewsPosts(locale) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not load news", error);
        return null;
    }
}
async function redeemPromoCode(playerId, rawCode) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not redeem promo code", error);
...
async function updateAccountDisplayName(playerId, authUserId, displayName) {
    ...
    `;
        ...
        if (typeof error === "object" && error && "code" in error && error.code === "23505")
            return "taken";
        (0, logging_1.logServerError)("Could not update account display name", error);
        return null;
    }
}
async function anonymizePlayerAccount(playerId) {
    if (!sql)
        return false;
    try {
        return await sql.begin(async (transaction) => {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not anonymize deleted account", error);
        return false;
    }
}
async function getDatabaseStatus() {
    ...
    try {
        ...
        (0, logging_1.logServerError)("Database health check failed", error);
        return "unavailable";
    }
}
...
async function canAccessPlayerData(playerId, authUserId) {
    if (!sql)
        return true;
    const [player] = await sql `
...
async function updatePlayerPresence(playerId, available) {
    ...
    try {
        ...
        (0, logging_1.logServerError)("Could not update player presence", error);
        return false;
    }
}
async function createPlayerChallenge(challengerId, challengedId, damageWager) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not create challenge", error);
...
async function getPlayerChallenges(playerId) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not load challenges", error);
        return null;
    }
}
async function respondToPlayerChallenge(playerId, challengeId, accept) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not respond to challenge", error);
    ...
    `;
        ...
        for (const row of rows) {
            ...
            if (direction === "blocked")
                ...
                response.outgoing.push(item);
            if (row.gift_id)
                response.unclaimedGiftCount += 1;
        }
        return response;
    }
    catch (error) {
        (0, logging_1.logServerError)("Could not load friends", error);
        return null;
    }
}
async function searchFriendPlayers(playerId, query) {
    if (!sql)
        return null;
    try {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not search friend players", error);
        return null;
    }
}
async function suggestFriendPlayers(playerId) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not suggest friend players", error);
        return null;
    }
}
async function sendFriendRequest(playerId, targetPlayerId) {
    if (!sql || playerId === targetPlayerId)
        return { ok: false, error: "Invalid player" };
    const [low, high] = [playerId, targetPlayerId].sort();
    try {
        return await sql.begin(async (transaction) => {
            const [target] = await transaction `select id from public.players where id = ${targetPlayerId} and account_type = 'registered'`;
            if (!target)
      ...
      `;
            if (existing?.status === "blocked")
                return { ok: false, error: "Friend request unavailable" };
            if (existing?.status === "accepted")
                return { ok: false, error: "You are already friends" };
            if (existing?.status === "pending")
                return { ok: false, error: existing.requested_by === playerId ? "Request already sent" : "This player already sent you a request" };
            await transaction `
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not send friend request", error);
        return { ok: false, error: "Could not send friend request" };
    }
}
async function respondToFriendRequest(playerId, friendshipId, accept) {
    if (!sql)
    ...
    try {
      ...
      `;
            if (!friendship || friendship.status !== "pending" || friendship.requested_by === playerId || ![friendship.player_low_id, friendship.player_high_id].includes(playerId)) {
                return { ok: false, error: "Friend request is no longer available" };
            }
            if (!accept) {
      ...
      `;
            if (counts.some((item) => item.count >= 25))
                return { ok: false, error: "The 25 friend limit has been reached" };
            await transaction `update public.friendships set status = 'accepted', responded_at = now() where id = ${friendshipId}`;
            return { ok: true };
        });
    }
    catch (error) {
        (0, logging_1.logServerError)("Could not respond to friend request", error);
...
async function updateFriendRelationship(playerId, friendshipId, action) {
    ...
    try {
        ...
        (0, logging_1.logServerError)("Could not remove or block friend", error);
        return { ok: false, error: "Could not update friendship" };
    }
}
async function sendFriendGift(playerId, friendshipId) {
    if (!sql)
    ...
    try {
      ...
      `;
            if (!friendship || friendship.status !== "accepted" || ![friendship.player_low_id, friendship.player_high_id].includes(playerId)) {
                return { ok: false, error: "You can only gift accepted friends" };
            }
      ...
      `;
            return inserted.length ? { ok: true } : { ok: false, error: "Gift already sent today" };
        });
    }
    catch (error) {
        (0, logging_1.logServerError)("Could not send friend gift", error);
        return { ok: false, error: "Could not send gift" };
    }
}
async function claimFriendGift(playerId, giftId) {
    if (!sql)
        return { ok: false, error: "Gifts are temporarily unavailable" };
    try {
        return await sql.begin(async (transaction) => {
            const [gift] = await transaction `
      ...
      `;
            if (!gift)
                return { ok: false, error: "Gift is no longer available" };
            await transaction `update public.friend_gifts set claimed_at = now() where id = ${giftId}`;
            const [updated] = await transaction `update public.players set stars = stars + ${gift.stars} where id = ${playerId} returning stars`;
            await transaction `
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not claim friend gift", error);
...
async function claimAllFriendGifts(playerId) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not claim all friend gifts", error);
...
async function linkPlayerAccount(guestPlayerId, displayName, authUserId, provider) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not link player account", error);
        return null;
    }
}
async function getPlayerCustomization(deviceId) {
    ...
    try {
      ...
      `.then((rows) => rows[0]),
        ...
        (0, logging_1.logServerError)("Could not load player customization", error);
        return null;
    }
}
...
async function acquireAndEquipCosmetic(deviceId, cosmeticId, cosmeticType, displayName) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not acquire or equip cosmetic", error);
        return null;
    }
}
async function reserveDamageWager(matchId, playerIds, stake) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not reserve Damage wager", error);
        return failed;
    }
}
async function settleDamageWager(matchId, winnerId) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not settle Damage wager", error);
    }
    return balances;
}
async function getRankedLeaderboard(deviceId) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not load ranked leaderboard", error);
        return null;
    }
}
...
async function upsertPlayer(deviceId, displayName) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not persist player profile", error);
        return null;
    }
}
async function getPlayerStars(deviceId) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not load player stars", error);
        return null;
    }
}
async function getDailyRewardStatus(deviceId) {
    ...
    `;
        ...
        (0, logging_1.logServerError)("Could not load daily reward status", error);
        return null;
    }
}
async function claimDailyReward(deviceId, displayName) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not claim daily reward", error);
        return null;
    }
}
async function saveCompletedMatch(match) {
    ...
    try {
      ...
      `;
        ...
        (0, logging_1.logServerError)("Could not persist completed match", error);
    }
    return progressUpdates;
}
